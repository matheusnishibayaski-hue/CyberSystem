...
import { Terminal, Lock, UserPlus, Mail, Key, Edit, Trash2, Ban, CheckCircle, Shield, UserCog } from "lucide-react";
...
export default function AdminLogin({ onBack }) {
  ...
  const handleSetRole = async (userId, role) => {
    try {
      await apiClient.patch(`/api/auth/admin/users/${userId}/role`, { role });
                                ...
                                {editingUser === user.id ? (
                                  ...
                                  <div className="space-y-3">
                                    ...
                                    <div className="text-xs text-green-700">
                                      PERFIL: <span className="text-green-300 font-bold">{(user.role || "viewer").toUpperCase()}</span>
                                    </div>
                                    <div className="grid grid-cols-3 gap-2 pt-2 border-t border-green-500/10">
                                      <button
                                        onClick={() => handleSetRole(user.id, "admin")}
                                        disabled={adminCount > 0 && user.role !== "admin"}
                                        className="border border-green-500/30 hover:bg-green-500/10 text-green-400 py-2 px-3 text-xs font-bold transition-all flex items-center justify-center gap-1 disabled:opacity-40"
                                      >
                                        <UserCog className="w-3 h-3" />
                                        ADMIN
                                      </button>
                                      <button
                                        onClick={() => handleSetRole(user.id, "security")}
                                        className="border border-green-500/30 hover:bg-green-500/10 text-green-400 py-2 px-3 text-xs font-bold transition-all flex items-center justify-center gap-1"
                                      >
                                        <Shield className="w-3 h-3" />
                                        SEGURANÇA
                                      </button>
                                      <button
                                        onClick={() => handleSetRole(user.id, "viewer")}
                                        className="border border-green-500/30 hover:bg-green-500/10 text-green-400 py-2 px-3 text-xs font-bold transition-all flex items-center justify-center gap-1"
                                      >
                                        VIEWER
                                      </button>
